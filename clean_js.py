...
# Define the full, clean JS block
# I'll include all features: nametags, animal chars, wobble, bot skills, improved UI logic, persistence
...
let selChar='glitch', scene, cam, ren, clk, sun;
...
var player, bots=[], lobbyMeshes=[], gameMeshes=[], stars=[], pads=[], solidPlatforms=[], hillMesh=null, interacts=[], particles3d=[], shakeAmt=0;
...
function showResults(){
  ...
  const nx=$('resNext'); if(nx){ nx.textContent=rGames.length>0&&rIdx<rGames.length-1?'NEXT RIFT':'PLAY AGAIN'; nx.onclick=()=>{hide('RES');if(rGames.length>0&&rIdx<rGames.length-1){rIdx++;setTimeout(()=>startMinigame(rGames[rIdx]),400);}else backToLobby();}; }
}
...
# Context: Everything between <script> and </script>
