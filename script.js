...
function addFriend(name, btn){
  ...
  btn.textContent = 'Friends ✅'; btn.classList.add('added'); btn.disabled=true;
}
...
(e.isPlayer ? '' : `<button class="friend-btn ${isFriend?'added':''}" onclick="addFriend('${e.name}', this)">${isFriend?'Friends ✅':'Add Friend'}</button>`);
pod.appendChild(d);
});
}
