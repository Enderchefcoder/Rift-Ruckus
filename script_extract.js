...
let player,bots=[],lobbyMeshes=[],gameMeshes=[],stars=[],pads=[];
...
function mat(c,r=.5){return sMeshStandardMaterial({color:c,roughness:r,metalness:.1});}
...
const gnd=sMesh(new THREE.CircleGeometry(110,60),mat(0x1a1a2e,.9));
gnd.rotation.x=-Math.PI/2;addMesh(gnd);
const plazaR=20,plazaH=2;
const plaza=sMesh(new THREE.CylinderGeometry(plazaR,plazaR,plazaH,32),mat(0x16213e,.4));
plaza.position.y=plazaH/2;addMesh(plaza);
solidPlatforms.push({mesh:plaza,radius:plazaR,topY:plazaH});
const g=new THREE.Group(); g.position.set(0,plazaH,0);
const cry=sMesh(new THREE.OctahedronGeometry(6),mat(0x00D4FF,.2));
cry.position.y=8; cry.userData.spin=true; cry.userData.bob=true; cry.userData.baseY=8; g.add(cry);
const lt=new THREE.PointLight(0x00D4FF,2,40); lt.position.y=8; g.add(lt);
addMesh(g);
...
const mirror=sMesh(new THREE.BoxGeometry(4.2,6,.15),sMeshStandardMaterial({color:0xE8EAF6,metalness:.95,roughness:.05}));
...
g.add(Object.assign(sMesh(new THREE.SphereGeometry(1.2,14,12),sm),{position:new THREE.Vector3(0,1.2,0)}));
g.add(Object.assign(sMesh(new THREE.SphereGeometry(.9,12,10),sm),{position:new THREE.Vector3(0,2.8,0)}));
g.add(Object.assign(sMesh(new THREE.SphereGeometry(.65,10,8),sm),{position:new THREE.Vector3(0,4,0)}));
...
case'jump':
  ...
  break;
case'stun':
  bots.forEach(b=>{if(d3(player.position,b.mesh.position)<12){b.stunned=2.5;b.vel.y=.2;mkParts(b.mesh.position.x,1.5,b.mesh.position.z,0xFFD700,20);}});
  shakeAmt=.8;
...
function buildHill(){
const mesh=sMesh(new THREE.CylinderGeometry(12,14,8,28),mat(0xFFD700,.35));
...
for(let x=0;x<sz;x++){
for(let z=0;z<sz;z++){
const pad=sMesh(new THREE.CylinderGeometry(4,4.3,1.2,16),mat(0x9C27B0,.35));
...
const sumoR = (curGame?.type==='sumo'?40:70);
if(bd>sumoR){
...
if(curGame?.type==='collect'||curGame?.type==='pads'){
const pd=d2(player.position,{x:0,z:0});
if(pd>70){player.position.x*=70/pd;player.position.z*=70/pd;}
}
runBotAI(dt);
...
const c=document.createElement('div');c.className='snow';c.textContent=['🎉','🎊','⭐','🎄','🎅'][Math.floor(Math.random()*5)];c.style.left=rnd(0,100)+'vw';c.style.fontSize='24px';c.style.animationDuration=rnd(3,5)+'s';$('PARTS')?.appendChild(c);setTimeout(()=>c.remove(),5000);
},i*40);
...
scene.background=new THREE.Color(0x1a2a4a);
scene.fog=new THREE.FogExp2(0x1a2a4a,.005);
gameMeshes.forEach(o=>scene.remove(o));gameMeshes=[];stars=[];pads=[];solidPlatforms=[];
// Re-add lobby platforms
lobbyMeshes.forEach(o=>o.visible=true);
solidPlatforms.push({mesh:lobbyMeshes[1],radius:20,topY:3}); // plaza
...
for(let i=0;i<n;i++){
const m=sMeshBasicMaterial({color:col,transparent:true,opacity:.9});
...
const c=curC();
const inp=joy.l;
const sprint=P.sprinting&&P.stamina>0?1.4:1;
if(P.sprinting&&P.stamina>0)P.stamina=Math.max(0,P.stamina-dt*30);
const spd=SPD*(c.spd/80)*sprint*.55;
const mag=Math.sqrt(inp.x**2+inp.y**2);
if(mag>.1){
const ang=Math.atan2(inp.x,inp.y)+camAng;
